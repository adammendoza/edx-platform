...
class AuthMixin(object):
    """
    Mixin that provides authentication and authorization helpers for APIView classes.
    """
    ...
    def user_can_access_course(self, user, course):
        ...
        """
        return settings.DEBUG or has_access(user, CourseStaffRole.ROLE, course) or has_access(user,
                                                                                              CourseInstructorRole.ROLE,
                                                                                              course)

    def check_course_permissions(self, request, course):
        """
        Checks if the request user can access the course.
        Raises PermissionDenied if the user does not have course acccess.
        """
        if not self.user_can_access_course(request.user, course):
            raise PermissionDenied


class CourseViewMixin(AuthMixin):
    """
    ...
    def get_course_or_404(self):
        ...
        try:
            ...
            self.check_course_permissions(self.request, course)
